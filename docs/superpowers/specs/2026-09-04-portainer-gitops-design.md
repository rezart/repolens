...
The stack is bootstrapped with the existing local image before the workflow is merged. Compose's `pull_policy: always` then pulls each newly published `main` image before deployment.
