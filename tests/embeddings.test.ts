...
describe('OpenAIEmbeddings', () => {
  ...
  it('splits 150 texts into batches of 16 and concatenates in order', async () => {
    const f = embeddingFetch();
    const e = new OpenAIEmbeddings({ baseUrl: 'http://x/v1', apiKey: 'k', model: 'm', fetch: f.fetch });
    ...
    expect(sizes).toEqual([16, 16, 16, 16, 16, 16, 16, 16, 16, 6]);
  ...
  it('reports one usage record per batch, carrying that batch prompt tokens', async () => {
    ...
    expect(seen).toEqual([160, 160, 160, 160, 160, 160, 160, 160, 20].map((prompt) => ({
      provider: 'embeddings',
      model: 'text-embed',
      inputTokens: prompt,
      cachedInputTokens: 0,
