...
export interface RepoCheckoutOptions {
  ...
  url: string;
  /** GitHub token; injected into the clone url and never echoed in errors. */
  token?: string;
  /** Override for tests. */
  git?: GitRunner;
}
...
export class RepoCheckout {
  ...
  /** Clone url with the token embedded, when one is configured. */
  authUrl(): string {
    if (!this.token) return this.url;
    if (!/^https?:\/\//.test(this.url)) return this.url;
    return this.url.replace(/^https?:\/\/(?:[^@/]+@)?/, `https://x-access-token:${this.token}@`);
  }
  ...
  async ensureClone(): Promise<void> {
    if (!existsSync(join(this.dir, '.git'))) {
      ...
      await this.git(['clone', '--no-checkout', this.authUrl(), this.dir], { cwd: dirname(this.dir) });
