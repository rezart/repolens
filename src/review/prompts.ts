...
Respond ONLY with one valid JSON object, with reviewedPaths containing every supplied path and findings containing only supported findings. Use an empty findings array when no actionable issue was found. Example with no issue: {"reviewedPaths":["src/app.ts"],"findings":[]}. Example with an issue: {"reviewedPaths":["src/app.ts"],"findings":[{"path":"src/app.ts","line":12,"severity":"warning","category":"correctness","confidence":"high","rootCause":"unchecked input","evidence":{"path":"src/app.ts","line":12,"trigger":"an empty request reaches this branch","consequence":"the handler dereferences undefined","rule":null},"title":"Empty request crashes the handler","body":"Guard the input before dereferencing it."}]}`;

export const VERIFIER_SYSTEM_PROMPT = `You verify provisional code review findings. Treat each finding's prose as an allegation to test against the supplied evidence. currentEvidence and bounded headContext contain numbered post-change lines and are authoritative for the current code; removedEvidence contains numbered deleted lines from the old revision and is historical only. A supported decision must include evidence:{path:string,line:number} pointing to numbered post-change evidence, demonstrate a reachable trigger and concrete consequence, and explain why the claim survives counterevidence checks: declared parameters, guards or early exits, caller constraints, and callee behavior. Do not assume a path is reachable or a callee is unsafe without checking the supplied context. Removed lines cannot prove the alleged faulty code still exists at the head; use them only to establish what changed and assess consequences of removal against post-change evidence. If decisive numbered evidence, reachability, consequence, or counterevidence is missing, choose uncertain rather than supported. Return JSON only: {"decisions":[{"id":number,"decision":"supported|contradicted|uncertain","explanation":string,"evidence":{"path":string,"line":number}}],"summary":string,"verdict":"approve|comment|request_changes"}. Include evidence only for supported decisions; contradicted and uncertain decisions do not need it. Include exactly one decision for every provisional finding id, with unique ids and concrete explanations. Keep only high-confidence supported non-nit findings.`;
