...
When a "Previous RepoLens review of this pull request" section is present, this is a re-review: build on it instead of starting over. Re-report a previous finding that still applies, at its current line number, preserving its title and rootCause so it can be matched to the earlier finding. Drop a previous finding that the "Changes to this file since the previous review" resolved. Drop a previous finding you now judge was wrong; do not keep it alive out of consistency. Discover new issues only in changes since the previous reviewed head. The full PR diff and unchanged callers are context, not a fresh discovery scope. Report a new issue on unchanged code only when a specific added or removed line in the latest delta demonstrably causes that failure; explain that causal link. Reassess each previous finding, but never equate its absence with a fix. If the delta is unavailable, explicitly fall back to reviewing the full PR. Use the "Commits in this pull request" list to understand how the change was built and which commits responded to the previous review, and the "Repository overview" to judge whether the change fits the architecture it lands in.
...
- A compatibility or API-availability finding requires a repository-declared supported version proving the API is unavailable.
- A security finding requires current evidence of an attacker-controlled input and the missing or bypassed protection on the reachable path.
...
function findingLines(findings: Finding[]): string {
  return findings.map((f) => `- [${f.severity}] ${f.path}:${f.line} — ${f.title}${f.rootCause ? ` (rootCause: ${JSON.stringify(f.rootCause)})` : ''}`).join('\n');
}
...
export function buildFileReviewMessage(input: {
  ...
  if (input.lineage) {
    ...
    if (l.previous) {
      ...
      parts.push(section('Latest PR delta (new discovery scope; other code is context)', clip(JSON.stringify(l.previous.delta), 12_000)));
    }
  }
...
export const VERIFIER_SYSTEM_PROMPT = `You verify provisional code review findings. Treat each finding's prose as an allegation to test against the supplied evidence. currentEvidence and structured headEvidence snippets with revision:"head" and numbered lines are authoritative for the current code; removedEvidence contains numbered deleted lines from the old revision and is historical only. If removedEvidence shows equivalent faulty behavior, the failure predates this pull request; decide contradicted unless the change demonstrably worsens it. Do not treat behavior selected by an explicit feature mode as a defect without a concrete violated contract or security boundary. Do not assume a value or DOM lookup is nullable unless authoritative evidence establishes a reachable missing-value case. Reject a compatibility or API-availability claim unless a repository-declared supported version proves the API is unavailable. Reject a security claim unless current evidence establishes an attacker-controlled input and the missing or bypassed protection on the reachable path. A claim that an API throws or rejects requires authoritative callee or contract evidence; the call site alone is insufficient. Supported and contradicted decisions must include evidence:{path:string,line:number} pointing to numbered post-change evidence; a contradicted decision may cite a guard or callee line rather than the finding line. A supported decision must demonstrate a concrete reachable failure under the supplied current evidence: a demonstrated attack or input and its concrete consequence. Reject generic security, performance, or limit warnings without that demonstrated trigger and consequence. Do not demand exhaustive whole-program proof when changed code plus authoritative head evidence directly establishes the trigger and consequence and no supplied counterevidence contradicts it. Explain why the claim survives counterevidence checks: declared parameters, guards or early exits, caller constraints, and callee behavior. Do not assume a path is reachable or a callee is unsafe without checking the supplied context. Removed lines cannot prove the alleged faulty code still exists at the head; use them only to establish what changed and assess consequences of removal against post-change evidence. If decisive numbered evidence, reachability, consequence, or counterevidence is missing, choose uncertain rather than supported or contradicted. On an uncertain decision, you may request one bounded missing-evidence lookup using missingEvidence:[{"symbol":string,"path"?:string,"question":string}]. The symbol is required and must identify the code target; path is optional, repository-relative, and unchanged by this pull request. Ask only a concise question needed to decide the supplied finding. Return JSON only: {"decisions":[{"id":number,"decision":"supported|contradicted|uncertain","explanation":string,"evidence":{"path":string,"line":number}}],"missingEvidence":[{"symbol":string,"path"?:string,"question":string}],"summary":string,"verdict":"approve|comment|request_changes"}. Include evidence for every supported or contradicted decision; uncertain decisions do not need it. Include exactly one decision for every provisional finding id, with unique ids and concrete explanations. Keep only high-confidence supported non-nit findings.`;
export const FOCUSED_VERIFIER_SYSTEM_PROMPT = `${VERIFIER_SYSTEM_PROMPT} This is a focused recheck of findings previously marked uncertain. Decide supported or contradicted only; do not return uncertain. Re-evaluate only the supplied findings and relevant current evidence, preserving the same citation requirements.`;

export const REGENERATION_GUIDANCE = 'Regenerate the complete response from scratch as concise, valid JSON with no continuation, preamble, or padding. Preserve and review all paths in reviewedPaths, report only supported distinct issues on allowed lines, and include all required evidence.';
